...
%% Simulation
theta_list=linspace(-pi/4+pi/(4*14),pi/4-pi/(4*14),16);
new_snr_list=zeros(length(theta_list),7);
count_list=zeros(length(theta_list),7);
nmse_list=zeros(length(theta_list),7);
parfor index=1:length(theta_list)
    snr_list_tem=zeros(1,7);
    count_list_tem=zeros(1,7);
    nmse_list_tem=zeros(1,7);
    theta=theta_list(index);
    for i=1:test_point_num
        y_coor=(rand*(y_max^3-y_min^3)+y_min^3)^(1/3);
        x_bound=(tan(theta))*y_coor;
        z_bound=x_bound;
        ...
        x_coor=x_coor*x_bound;
        z_coor=z_coor*z_bound;
...
save('data14.mat','theta_list','new_snr_list','count_list','nmse_list','test_point_num','snr','num_x','num_z','M','y_min','y_max','grid_spacing');
toc;
