function [new_snr,count,nmse]=method_dft(num_x,num_z,M,lambda,antenna_coor,noise_power,channel,y_list,y_list_index,refine_length)
num_all=num_x*num_z;
%% Phase I-1
pilot_powers=zeros(2^M,1);
for m=1:2^M
    partition_words=zeros(1,num_all);
    sin_theta=(2*m-1)/(2^M)-1;
    partition_words((1:num_z:num_all)+num_z/2)=exp(-1j*pi*sin_theta*(1:num_x))/sqrt(num_x);
    pure_pilot=partition_words*channel;
    noise=sqrt(noise_power)*(randn+1j*randn)/sqrt(2);
    pilot_powers(m)=abs(pure_pilot+noise);
end
[~,x_index]=max(pilot_powers);
%% Phase I-2
pilot_powers=zeros(2^M,1);
for m=1:2^M
    partition_words=zeros(1,num_all);
    sin_theta=(2*m-1)/(2^M)-1;
    partition_words(num_all/2+(1:num_z))=exp(-1j*pi*sin_theta*(1:num_z))/sqrt(num_z);
    pure_pilot=partition_words*channel;
    noise=sqrt(noise_power)*(randn+1j*randn)/sqrt(2);
    pilot_powers(m)=abs(pure_pilot+noise);
end
[~,z_index]=max(pilot_powers);

%% Phase II
estimate_sin_x=min(sqrt(2)/2,max(-sqrt(2)/2,(2*x_index-1)/2^M-1));
estimate_sin_z=min(sqrt(2)/2,max(-sqrt(2)/2,(2*z_index-1)/2^M-1));
