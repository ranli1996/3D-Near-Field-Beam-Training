...
new_snr_list=zeros(length(snr_list),2*5);
count_list=zeros(length(snr_list),2*5);
nmse_list=zeros(length(snr_list),2*5);
parfor index=1:length(snr_list)
    snr_list_tem=zeros(1,2*5);
    count_list_tem=zeros(1,2*5);
    nmse_list_tem=zeros(1,2*5);
    snr=snr_list(index);
    ...
    for i=1:test_point_num
        ...
        [new_snr,count,nmse]=method_mirror_2_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,2);
        snr_list_tem(2)=snr_list_tem(2)+new_snr;
        count_list_tem(2)=count_list_tem(2)+count;
        nmse_list_tem(2)=nmse_list_tem(2)+nmse;

        [new_snr,count,nmse]=method_mirror_2_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,3);
        snr_list_tem(3)=snr_list_tem(3)+new_snr;
        count_list_tem(3)=count_list_tem(3)+count;
        nmse_list_tem(3)=nmse_list_tem(3)+nmse;

        [new_snr,count,nmse]=method_mirror_2_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,5);
        snr_list_tem(4)=snr_list_tem(4)+new_snr;
        count_list_tem(4)=count_list_tem(4)+count;
        nmse_list_tem(4)=nmse_list_tem(4)+nmse;

        % [new_snr,count,nmse]=method_mirror_2_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,8);
        ...
        count_list_tem(6)=count_list_tem(6)+count;
        nmse_list_tem(6)=nmse_list_tem(6)+nmse;

        [new_snr,count,nmse]=method_mirror_3_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,2);
        snr_list_tem(7)=snr_list_tem(7)+new_snr;
        count_list_tem(7)=count_list_tem(7)+count;
        nmse_list_tem(7)=nmse_list_tem(7)+nmse;

        [new_snr,count,nmse]=method_mirror_3_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,3);
        snr_list_tem(8)=snr_list_tem(8)+new_snr;
        count_list_tem(8)=count_list_tem(8)+count;
        nmse_list_tem(8)=nmse_list_tem(8)+nmse;

        [new_snr,count,nmse]=method_mirror_3_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,5);
        snr_list_tem(9)=snr_list_tem(9)+new_snr;
        count_list_tem(9)=count_list_tem(9)+count;
        nmse_list_tem(9)=nmse_list_tem(9)+nmse;

        % [new_snr,count,nmse]=method_mirror_3_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,8);
        % snr_list_tem(10)=snr_list_tem(10)+new_snr;
        % count_list_tem(10)=count_list_tem(10)+count;
        % nmse_list_tem(10)=nmse_list_tem(10)+nmse;
    end
...
save('data71.mat','snr_list','new_snr_list','num_x','num_z');
toc;
