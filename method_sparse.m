function [new_snr,count,nmse]=method_sparse(num_x,num_z,M,lambda,antenna_coor,noise_power,channel,y_list,y_list_index,refine_length)
num_all=num_x*num_z;
%% Phase I-1
x_indices=ones(M+1,1);
partition_words=zeros(2,num_all);
for m=1:(log2(num_x)-1)
    x_index_1=2*x_indices(m)-1;
    sin_theta=(2*x_index_1-1)/(2^m)-1;
    partition_words(1,((1+num_z*(num_x/2-2^(m-1))):num_z:(1+num_z*(num_x/2+2^(m-1)-1)))+(num_z/2-1))=exp(-1j*pi*sin_theta*(1:2^m))/sqrt(2^m);

    x_index_2=2*x_indices(m);
    sin_theta=(2*x_index_2-1)/(2^m)-1;
    partition_words(2,((1+num_z*(num_x/2-2^(m-1))):num_z:(1+num_z*(num_x/2+2^(m-1)-1)))+(num_z/2-1))=exp(-1j*pi*sin_theta*(1:2^m))/sqrt(2^m);
...
for m=log2(num_x):M
    x_index_1=2*x_indices(m)-1;
    sin_theta=(2*x_index_1-1)/(2^m)-1;
    partition_words(1,(1:num_z:num_all)+(num_z/2-1))=exp(-1j*pi*sin_theta*(1:num_x))/sqrt(num_x);

    x_index_2=2*x_indices(m);
    sin_theta=(2*x_index_2-1)/(2^m)-1;
    partition_words(2,(1:num_z:num_all)+(num_z/2-1))=exp(-1j*pi*sin_theta*(1:num_x))/sqrt(num_x);
...
for m=1:(log2(num_z)-1)
    z_index_1=2*z_indices(m)-1;
    sin_theta=(2*z_index_1-1)/(2^m)-1;
    partition_words(1,1+((num_z/2-2^(m-1)):(num_z/2+2^(m-1)-1))+(num_all/2))=exp(-1j*pi*sin_theta*(1:2^m))/sqrt(2^m);

    z_index_2=2*z_indices(m);
    sin_theta=(2*z_index_2-1)/(2^m)-1;
    partition_words(2,1+((num_z/2-2^(m-1)):(num_z/2+2^(m-1)-1))+(num_all/2))=exp(-1j*pi*sin_theta*(1:2^m))/sqrt(2^m);
...
%% Phase II
estimate_sin_x=min(sqrt(2)/2,max(-sqrt(2)/2,(2*x_indices(M+1)-1)/2^M-1));
estimate_sin_z=min(sqrt(2)/2,max(-sqrt(2)/2,(2*z_indices(M+1)-1)/2^M-1));
