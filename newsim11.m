...
% grid_count=method_grid_basic(aperture_x,aperture_z,grid_spacing,y_min,y_max+5);

%% Simulation
y_sample_list=linspace(0,1,15);
new_snr_list=zeros(length(y_sample_list),7);
count_list=zeros(length(y_sample_list),7);
nmse_list=zeros(length(y_sample_list),7);
parfor index=1:length(y_sample_list)
    % fprintf('%.2f\n',index/length(y_sample_list))
    snr_list_tem=zeros(1,7);
    count_list_tem=zeros(1,7);
    nmse_list_tem=zeros(1,7);
    y_sample=y_sample_list(index);
    for i=1:test_point_num
        y_coor=y_min+(y_max-y_min)*y_sample;
        x_coor=(2*rand-1)*y_coor;
        z_coor=(2*rand-1)*y_coor;
...
save('data11.mat','y_sample_list','new_snr_list','count_list','nmse_list','test_point_num','snr','num_x','num_z','M','y_min','y_max','grid_spacing');
toc;
