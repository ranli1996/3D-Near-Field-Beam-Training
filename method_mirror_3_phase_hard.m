function [new_snr,count,nmse]=method_mirror_3_phase_hard(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,vary)
num_all=num_x*num_z;
%% Phase I-1
...
for m=1:M
    for x=1:2
        ...
        mirror_words(x,:)=mirror_words(x,:).*sqrt(1+(2*rand(1,num_all)-1)*vary);
...
for m=1:M
    for z=1:2
        ...
        mirror_words(z,:)=mirror_words(z,:).*sqrt(1+(2*rand(1,num_all)-1)*vary);
    ...
    for tem_x=1:(min(2^M,estimate_x_index+pre_length)-max(1,estimate_x_index-pre_length)+1)
        for tem_z=1:(min(2^M,estimate_z_index+pre_length)-max(1,estimate_z_index-pre_length)+1)
            if iden_matrix(tem_x,tem_z)>0
                ...
                focus_word=exp(1j*2*pi*focus_distance'/lambda)/sqrt(num_all).*sqrt(1+(2*rand(1,num_all)-1)*vary);
