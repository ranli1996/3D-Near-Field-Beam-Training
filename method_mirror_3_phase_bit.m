function [new_snr,count,nmse]=method_mirror_3_phase_bit(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,bit)
num_all=num_x*num_z;
%% Phase I-1
...
for m=1:M
    for x=1:2
        ...
        tem_word=exp(-1j*2*pi*virtual_distance'/lambda);
        mirror_words(x,((1:num_z:end)+num_z/2-1))=tem_word((1:num_z:end)+num_z/2-1);
        mirror_words(x,:)=quantize_phase(mirror_words(x,:),bit)/sqrt(num_x);
...
for m=1:M
    for z=1:2
        ...
        mirror_words(z,:)=quantize_phase(mirror_words(z,:),bit)/sqrt(num_z);
    ...
    for tem_x=1:(min(2^M,estimate_x_index+pre_length)-max(1,estimate_x_index-pre_length)+1)
        for tem_z=1:(min(2^M,estimate_z_index+pre_length)-max(1,estimate_z_index-pre_length)+1)
            if iden_matrix(tem_x,tem_z)>0
                ...
                focus_word=exp(1j*2*pi*focus_distance'/lambda);
                focus_word=quantize_phase(focus_word,bit)/sqrt(num_all);
...
function x_q = quantize_phase(x, B)
L = 2^B;              % number of quantization levels
delta = 2*pi / L;     % phase step
phase = angle(x);                 % [-pi, pi]
phase_q = delta * round(phase / delta);
x_q = exp(1j * phase_q);
end