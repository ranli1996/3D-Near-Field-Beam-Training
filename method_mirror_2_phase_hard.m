function [new_snr,count,nmse]=method_mirror_2_phase_hard(num_x,num_z,M,lambda,antenna_coor,aperture_x,aperture_z,noise_power,channel,y_list,y_list_index,vary)
...
for m=1:M
    for x=1:2
        for z=1:2
            ...
            mirror_words(point_index,:)=exp(-1j*2*pi*virtual_distance'/lambda)/sqrt(num_all).*sqrt(1+(2*rand(1,num_all)-1)*vary);
